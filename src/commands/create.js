...
async function create(dirPathName, fileName, content) {
  ...
  try {
    await checkDirExist(dirPath).then(async (dirExists) => {
      if (!dirExists) {
    ...
    await checkFileExists(filePath).then(async (fileExists) => {
      if (fileExists) {
        throw new Error(
          `FS operation failed. File "${filePath}" already exists!`,
        );
      }
    });
    ...
    console.log(
      `File "${fileName}" has been successfully created in the "${dirPathName}" directory!`,
    );
  } catch (error) {

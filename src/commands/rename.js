...
async function remane(srcFileName, destFileName) {
  try {
    ...
    await checkFileExists(srcFilePath).then(async (fileExists) => {
      if (!fileExists) {
        throw new Error(
          `FS operation failed. File "${srcFileName}" does not exist!`,
        );
      }
    });
    ...
    await checkFileExists(destFilePath).then(async (fileExists) => {
      if (fileExists) {
        throw new Error(
          `FS operation failed. File "${destFileName}" already exists!`,
        );
      }
    });
    ...
    console.log(
      `File "${srcFileName}" has been successfully renamed to "${destFileName}"!`,
    );
  } catch (error) {

...
async function copy(srcDirName, destDirName) {
  try {
    ...
    await checkDirExist(scrDirPath).then(async (dirExists) => {
      if (!dirExists) {
    ...
    await checkDirExist(destDirPath).then(async (dirExists) => {
      if (!dirExists) {
        await createDir(destDirPath);
      } else {
        throw new Error(
          `FS operation failed. Folder "${destDirName}" already exists!`,
        );
      }
    });
    ...
    console.log(
      `Folder "${srcDirName}" has been successfully copied to "${destDirName}"!`,
    );
  } catch (error) {
